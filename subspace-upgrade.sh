...
LAST_BUILD_MONTH_DAY=$(curl https://api.github.com/repos/autonomys/subspace/releases | grep name | grep \"mainnet- | grep $(date +%Y) | cut -d : -f2 | cut -d - -f4,5,6 | cut -d \" -f1 | sort -M | tail -n1)
