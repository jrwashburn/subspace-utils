...
LATEST_NODE=$(curl https://api.github.com/repos/subspace/subspace/releases | grep $(date +%Y-$LAST_BUILD_MONTH_DAY) | grep browser_download_url | grep $PLATFORM-v$CPULEVEL |  grep ubuntu | grep node | cut -d : -f2,3 |  tr -d ' "')
LATEST_FARMER=$(curl https://api.github.com/repos/subspace/subspace/releases | grep $(date +%Y-$LAST_BUILD_MONTH_DAY) | grep browser_download_url | grep $PLATFORM-v$CPULEVEL | grep ubuntu | grep farmer | cut -d : -f2,3 |  tr -d ' "')
